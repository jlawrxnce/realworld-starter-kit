...
class Routes {
  ...
  async register(session: WebSessionDoc, user: UserMessage) {
    ...
    const token = await Jwt.create(account._id, account.username);
    WebSession.start(session, account._id);
    return { user: Merge.createUserMessage(account, profile, token) };
  }
  ...
  async login(session: WebSessionDoc, user: UserMessage) {
    ...
    return { user: Merge.createUserMessage(account, profile) };
  }

  @Router.get("/user")
  async getUser(session: WebSessionDoc) {
    const user = WebSession.getUser(session);
    const account = Account.getAccountById(user);
    const profile = Profile.getProfileById(user);
    return { user: Merge.createUserMessage(await account, await profile) };
  }

  @Router.put("/user")
  async updateUser(session: WebSessionDoc, user: UserMessage) {
    const _id = WebSession.getUser(session);
    const account = Account.update(_id, { ...user });
    const profile = Profile.update(_id, { ...user });
    return { user: Merge.createUserMessage(await account, await profile) };
  }
  ...
  async listArticles(session: WebSessionDoc, tag: string, author: string, favorited: string, limit: number, offset: number) {
    ...
    let tagArticleIds: Set<ObjectId>;
    if (tag) {
      tagArticleIds = new Set((await Tag.getTagByContent(tag)).map((tag) => tag.target));
    }

    let favoriteIds: Set<ObjectId>;
    if (favorited) {
      favoriteIds = new Set((await Favorite.getFavorites({ userId })).map((favorite) => favorite.target));
    }

    // TODO: have to do manual filtering here (very bad)
    articles = articles.filter((article) => {
      let filtered = true;
      if (author) filtered = filtered && article.author == authorId;
      if (tag) filtered = filtered && tagArticleIds.has(article._id);
      if (favorited) filtered = filtered && favoriteIds.has(article._id);
    ...
    const articleMessages = await Promise.all(
      articles.map(async (article) => {
        ...
        return Merge.createArticleMessage(article, profileMessage, tagList, false, favoritesCount);
      }),
    );
  ...
  async getFeedArticles(session: WebSessionDoc, limit: number, offset: number) {
    ...
    const articleMessages = await Promise.all(
      articles.map(async (article) => {
        ...
        const favoritesCount = await Favorite.countTargetFavorites(article._id);
        const tagList = await Tag.getTagByTarget(article._id).then(Tag.stringify);
        return Merge.createArticleMessage(article, profileMessage, tagList, true, favoritesCount);
      }),
    );
  ...
  async addComment(comment: CommentDoc, slug: string) {
    const article = await Article.getBySlugOrThrow(slug);
    const newComment = await Comment.create(comment.author, article?._id, comment.body);
