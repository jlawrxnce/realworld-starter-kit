...
export default class MembershipConcept {
  ...
  async getByOwner(owner: ObjectId) {
    ...
    if (!membership) {
      throw new NotFoundError(`Membership not found for user!`);
    }
    return membership;
  }

  async updateMembership(owner: ObjectId, tier: Tier, autoRenew: boolean) {
    const membership = await this.getByOwner(owner);
    if (membership.tier === Tier.Free && tier === Tier.Free) {
      throw new NotAllowedError("Free users cannot update membership!");
    }
    await this.memberships.deleteOne({ owner });
    const _id = await this.memberships.createOne({ owner, tier, autoRenew, renewalDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) });
    return await this.memberships.readOne({ _id });
  }
